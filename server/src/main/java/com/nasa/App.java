...
public class App extends NanoHTTPD {
	...
	public Response serve(IHTTPSession session) {
		...
		for (int i = 0; i < rr.getThresholdSize(); i++) {
			...
			// If the shortest path is not NULL, write the node list with distances to
			// console
			if (nodes == null) {
			...
			/*
			 * use nodes to process shortest path and return a json array of routes
			 * documented in architecture document. For example, [ { nodes: [list of node
			 * ids], ...otherMetaDataProperties } ]
			 */
